...
bool CMisc2D::Init(void)
{
	...
	if (cMap2D->FindValue(400, uiRow, uiCol) == true)
	{
		miscType = COIN;
		// Erase the value of the player in the arrMapInfo
		cMap2D->SetMapInfo(uiRow, uiCol, 2);
	}
	else if (cMap2D->FindValue(401, uiRow, uiCol) == true)
	{
		miscType = BONE;
		// Erase the value of the player in the arrMapInfo
	...
	if (miscType == COIN)
	{
		...
		iTextureID = CImageLoader::GetInstance()->LoadTextureGetID("Image/Coin.png", true);
		if (iTextureID == 0)
		{
			cout << "Image/Coin.png" << endl;
			return false;
		}
	}
	else if (miscType == BONE)
	{
		// Load the enemy2D texture
	...
	animatedMisc = CMeshBuilder::GenerateSpriteAnimation(1, 4, cSettings->TILE_WIDTH, cSettings->TILE_HEIGHT);
	animatedMisc->AddAnimation("idle", 0, 3);
	//CS: Play the "idle" animation as default
	animatedMisc->PlayAnimation("idle", -1, 1.0f);
 ...
 */
void CMisc2D::Update(const double dElapsedTime)
{
	if (!bIsActive)
		return;
	...
	animatedMisc->Update(dElapsedTime);
...
*/
void CMisc2D::Seti32vec2Index(const int iIndex_XAxis, const int iIndex_YAxis)
{
...
*/
void CMisc2D::Seti32vec2NumMicroSteps(const int iNumMicroSteps_XAxis, const int iNumMicroSteps_YAxis)
{
 ...
 */
void CMisc2D::SetPlayer2D(CPlayer2D* cPlayer2D)
{
 ...
 */
void CMisc2D::Constraint(DIRECTION eDirection)
{
	if (eDirection == LEFT)
	{
		if (vec2Index.x < 0)
	...
	{
		cout << "CMisc2D::Constraint: Unknown direction." << endl;
	}
}
...
/**
 ...
 @param eDirection A DIRECTION enumerated data type which indicates the direction to check
 */
bool CMisc2D::CheckPosition(DIRECTION eDirection)
{
	if (eDirection == LEFT)
	{
	...
	{
		cout << "CMisc2D::CheckPosition: Unknown direction." << endl;
	}

	return true;
}

bool CMisc2D::AdjustPosition(DIRECTION eDirection)
{
	if (eDirection == LEFT)
	{
		// If the new position is between 2 rows, then check both rows as well
 ...
 */
bool CMisc2D::InteractWithPlayer(void)
{
 ...
 */
void CMisc2D::FlipHorizontalDirection(void)
{
...
*/
void CMisc2D::UpdatePosition(void)
{
