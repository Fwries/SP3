...
/**
 @brief Constructor
 */
CGameLoseState::CGameLoseState(void)
	: background(NULL)
{
 ...
 */
bool CGameLoseState::Init(void)
{
	cout << "CGameLoseState::Init()\n" << endl;

	CShaderManager::GetInstance()->Use("Shader2D");
	//CShaderManager::GetInstance()->activeShader->setInt("texture1", 0);

	//Create Background Entity
	background = new CBackgroundEntity("Image/MenuBackgroundL.png");
 ...
 */
bool CGameLoseState::Update(const double dElapsedTime)
{
	...
	// 2. Show a simple window that we create ourselves. We use a Begin/End pair to created a named window.
	{
		...
		if (CKeyboardController::GetInstance()->IsKeyReleased(GLFW_KEY_ESCAPE))
		{
			...
			// Load the menu state
			cout << "Quitting the game from GameEndState" << endl;
 ...
 */
void CGameLoseState::Render(void)
{
	...
	//cout << "CGameLoseState::Render()\n" << endl;
}

/**
 @brief Destroy this class instance
 */
void CGameLoseState::Destroy(void)
{
	// Disable the cursor
	if (CSettings::GetInstance()->bDisableMousePointer == true)
