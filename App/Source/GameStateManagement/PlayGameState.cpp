...
bool CPlayGameState::Update(const double dElapsedTime)
{
	...
	if (!cScene2D->Update(dElapsedTime))
	{
		cSoundController->StopPlayByID(1);
		...
		if (cScene2D->GetPlayerWon() == false)
		{
			// Load the menu state
			cout << "Loading GameLoseState" << endl;
			CGameStateManager::GetInstance()->SetActiveGameState("GameLoseState");
			CGameStateManager::GetInstance()->OffPauseGameState();
			return true;
		}
		else
		{
			// Load the menu state
			cout << "Loading GameWinState" << endl;
